...
//	        driver.findElement(By.xpath(OR.getProperty(id)));
	...
	public static boolean waitForElementDisplayed(String object, String data) throws Exception{
		int i = 0;
		while (!elementDisplayed(By.xpath(OR.getProperty(object)))&&(i <= 120)){
...
//			  }
			  ...
			  Assert.assertTrue(driver.findElement(By.xpath(OR.getProperty("txtbx_Status"))).getAttribute("value").trim().toUpperCase().equals(status), "Assertion failed - status not changed");
		  }catch(AssertionError ae){
				Log.error("Assertion failed --- " + ae.getMessage());
				DriverScript.bResult = false;	
		  } catch(Exception e) {
